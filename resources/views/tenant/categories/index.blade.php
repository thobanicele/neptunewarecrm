    ...
    <div class="container-fluid py-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div class="d-flex gap-2">
                @can('create', \App\Models\Category::class)
                    <a href="{{ tenant_route('tenant.categories.create') }}" class="btn btn-primary">+ New Category</a>
                @endcan
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                {{-- Filters (match Products) --}}
                <form class="row g-2 mb-3" method="GET" action="{{ tenant_route('tenant.categories.index') }}">
                    ...
                    <div class="col-6 col-lg-2">
                        <select class="form-select" name="sort" onchange="this.form.submit()">
                            @php
                                ...
                                $dir = $dir ?? 'asc';
                            @endphp
                            <option value="sort_order" @selected($sort === 'sort_order')>Sort: Order</option>
                    ...
                    <div class="col-12 col-lg-auto d-flex gap-2">
                        ...
                        <a class="btn btn-light" href="{{ tenant_route('tenant.categories.index') }}">Reset</a>
                    </div>
                </form>
                ...
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <div class="text-muted small">
                        Showing <b>{{ $categories->firstItem() ?? 0 }}</b>–<b>{{ $categories->lastItem() ?? 0 }}</b>
                        of <b>{{ $categories->total() }}</b> categories
                    </div>

                    @if (($q ?? '') !== '' || ($status ?? '') !== '' || ($sort ?? '') !== '' || ($dir ?? '') !== '')
                        <a class="small text-decoration-none" href="{{ tenant_route('tenant.categories.index') }}">
                            Clear filters
                        </a>
                    @endif
                </div>
                    ...
                    <table class="table table-hover table-sm align-middle mb-0">
                        <thead>
                            <tr>
                                ...
                                <th>Parent</th>
                                <x-index.th-sort label="Order" key="sort_order" :sort="$sort" :dir="$dir" />
                                        ...
                                        <div class="btn-group">
                                            @can('update', $c)
                                                <a class="btn btn-sm btn-outline-primary"
                                                    href="{{ tenant_route('tenant.categories.edit', ['category' => $c->id]) }}">
                                                    Edit
                                                </a>
                                            @else
                                                <a class="btn btn-sm btn-outline-primary disabled" href="#">Edit</a>
                                            @endcan
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                ...
                                                <li>
                                                    @can('delete', $c)
                                                        <form method="POST"
                                                            action="{{ tenant_route('tenant.categories.destroy', ['category' => $c->id]) }}"
