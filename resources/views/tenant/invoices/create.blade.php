...
@section('content')
    <div class="container-fluid py-4">
        ...
        <form method="POST" action="{{ tenant_route('tenant.invoices.store') }}">
            @csrf

            {{-- TOP: Invoice Header + Customer Details --}}
            <div class="card mb-3">
                <div class="card-body">

                    {{-- Header row: Logo + invoice meta --}}
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="border rounded p-3">
                                <div class="row g-2">
                                    ...
                                    <div class="col-12">
                                        <div class="text-muted small">Reference (optional)</div>
                                        <input type="text" class="form-control" name="reference"
                                            value="{{ old('reference') }}" maxlength="120"
                                            placeholder="e.g. PO-12345 / Ref-9876">
                                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-lg-8">
                            ...
                            <select class="form-select" name="company_id" id="companySelect"
                                ...
                                data-prefill-contact-id="{{ old('contact_id', $prefillContactId ?? '') }}" required>
                    ...
                    <div class="row g-3 mt-2">
                        <div class="col-12 col-lg-6">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <div class="text-muted small fw-semibold">BILL TO</div>
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="text-muted small fw-semibold mb-1">SHIP TO</div>
                        ...
                        <div class="col-12 col-lg-4">
                            ...
                            <select class="form-select" name="tax_type_id" id="defaultTaxType">
                                ...
                                @foreach ($taxTypes ?? collect() as $t)
                                    <option value="{{ data_get($t, 'id') }}" @selected((string) old('tax_type_id') === (string) data_get($t, 'id'))>
                                        {{ data_get($t, 'name') }}
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-4">
                        <div class="col-12 col-lg-6">
                            ...
                            <textarea class="form-control" name="notes" rows="4" placeholder="Will be displayed on the invoice">{{ old('notes') }}</textarea>
                    ...
                    <div class="d-flex gap-2 mt-3">
                        ...
                        <a class="btn btn-light" href="{{ tenant_route('tenant.invoices.index') }}">Cancel</a>
                    </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const prefillContactId = (companySelect.getAttribute('data-prefill-contact-id') || '').toString();
            ...
            async function loadContactsForCompany(companyId, selectedId = '') {
                ...
                if (!url || url.includes('__ID__')) {
                    console.error('[NW] contacts url template missing/invalid');
                    return;
                }
                ...
                try {
                    const res = await fetch(url + `?selected_id=${encodeURIComponent(existingSelected)}`, {
                        headers: {
                    ...
                    // if selected isn't in the company list, fallback to first contact
            ...
            // On change company -> reload contacts
            companySelect.addEventListener('change', function() {
                loadContactsForCompany(companySelect.value);
            });

            // On page load: if company prefilled, load contacts and select prefill contact
            if ((companySelect.value || '').toString().trim()) {
        ...
        (function() {
            ...
            function setSelectedUI(row, selected) {
                ...
                if (selected) {
                    ...
                    if (pickedWrap) pickedWrap.style.display = 'block';
                    if (descWrap) descWrap.style.display = 'block';
                } else {
                    if (searchWrap) searchWrap.style.display = 'block';
                    if (pickedWrap) pickedWrap.style.display = 'none';
            ...
            function reindex() {
                itemsBody.querySelectorAll('.invoice-item-row').forEach((row, idx) => {
            ...
            function addRow(prefill = {}) {
                const idx = itemsBody.querySelectorAll('.invoice-item-row').length;
                ...
                <td>
                    <div class="nw-picker">
                        ...
                        <input type="hidden" name="items[${idx}][unit]" value="">
            ...
            defaultTaxType?.addEventListener('change', () => {
                itemsBody.querySelectorAll('.invoice-item-row .taxType').forEach(sel => {
