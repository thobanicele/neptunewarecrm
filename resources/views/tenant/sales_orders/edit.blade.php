...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                ...
                <div class="text-muted small">
                    {{ $salesOrder->sales_order_number }} • Tenant: {{ $tenant->name }} ({{ $tenant->subdomain }})
                </div>
            </div>

            <div class="d-flex gap-2">
                <a href="{{ tenant_route('tenant.sales-orders.show', ['salesOrder' => $salesOrder->id]) }}"
                    class="btn btn-light">Back</a>
                <button class="btn btn-primary" type="submit" form="soForm">Save changes</button>
            </div>
        </div>
        ...
        <form id="soForm" method="POST"
            action="{{ tenant_route('tenant.sales-orders.update', ['salesOrder' => $salesOrder->id]) }}">
            @csrf
            @method('PUT')

            {{-- TOP: Header + Customer Details (same as Quotes) --}}
            <div class="card mb-3">
                <div class="card-body">
                                ...
                                <div class="row g-2">
                                    <div class="col-6">
                                        ...
                                        <input type="date" class="form-control" name="issued_at"
                                            value="{{ old('issued_at', optional($salesOrder->issued_at)->toDateString() ?: now()->toDateString()) }}">
                                    </div>

                                    <div class="col-6">
                                        <div class="text-muted small">Currency</div>
                                        <input type="text" class="form-control" name="currency" maxlength="10"
                                            value="{{ old('currency', $salesOrder->currency ?: 'ZAR') }}">
                                    </div>

                                    <div class="col-6">
                                        <div class="text-muted small">Status</div>
                                        <select class="form-select" disabled>
                                            <option selected>{{ strtoupper((string) $salesOrder->status) }}</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-6">
                                        ...
                                        <select class="form-select @error('sales_person_user_id') is-invalid @enderror"
                                            ...
                                            @foreach ($salesPeople ?? collect() as $u)
                                                <option value="{{ data_get($u, 'id') }}" @selected((string) old('sales_person_user_id', $salesOrder->sales_person_user_id) === (string) data_get($u, 'id'))>
                                                    {{ data_get($u, 'name') }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select class="form-select" name="deal_id" id="dealSelect">
                                            ...
                                            @foreach ($deals ?? collect() as $d)
                                                <option value="{{ data_get($d, 'id') }}" @selected((string) old('deal_id', $salesOrder->deal_id) === (string) data_get($d, 'id'))>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" class="form-control" name="reference" maxlength="120"
                                            ...
                                            placeholder="e.g. PO / Ref / Internal">
                                    </div>
                                </div>
                            </div>
                            <div class="form-text">Owner is who created it; Salesperson gets credit.</div>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-lg-8">
                            ...
                            <select class="form-select @error('company_id') is-invalid @enderror" name="company_id"
                                ...
                                data-prefill-contact-id="{{ (string) old('contact_id', $salesOrder->contact_id) }}"
                                data-prefill-company-id="{{ (string) old('company_id', $salesOrder->company_id) }}"
                                required>
                                <option value="">— select company —</option>
                                @foreach ($companies ?? collect() as $c)
                                    <option value="{{ data_get($c, 'id') }}" @selected((string) old('company_id', $salesOrder->company_id) === (string) data_get($c, 'id'))>
                                        {{ data_get($c, 'name') }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12 col-lg-4">
                            ...
                            <select class="form-select" name="tax_type_id" id="defaultTaxType">
                                @foreach ($taxTypes ?? collect() as $t)
                                    <option value="{{ data_get($t, 'id') }}" @selected((string) old('tax_type_id', $salesOrder->tax_type_id) === (string) data_get($t, 'id'))>
                                        {{ data_get($t, 'name') }}
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-4">
                        <div class="col-12 col-lg-6">
                            ...
                            <textarea class="form-control" name="notes" rows="4">{{ old('notes', $salesOrder->notes) }}</textarea>

                            <div class="mt-3">
                                <label class="form-label">Terms</label>
                                <textarea class="form-control" name="terms" rows="4">{{ old('terms', $salesOrder->terms) }}</textarea>
                            </div>
                    ...
                    <div class="d-flex gap-2 mt-3">
                        ...
                        <a class="btn btn-light"
                            href="{{ tenant_route('tenant.sales-orders.show', ['salesOrder' => $salesOrder->id]) }}">Cancel</a>
                    </div>
...
@push('scripts')
    @php
        ...
        if (!$seedItems) {
            $seedItems = ($salesOrder->items ?? collect())
                ->map(function ($it) {
                    return [
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // A) CONTACTS loader (company -> contacts)
            ...
            // C) Items picker table (same as Quotes)
            (function initItems() {
