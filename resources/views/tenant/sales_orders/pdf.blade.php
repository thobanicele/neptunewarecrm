...
<body>
    @php
        ...
        // Legacy fallbacks if address table is empty
        ...
        $billing = $billSnap ?: ($billingFromTable ?: $billingFallback);
        $shipping = $shipSnap ?: ($shippingFromTable ?: $shippingFallback);

        $paymentTerms = trim((string) ($company?->payment_terms ?? ''));

        // Totals (prefer header totals)
        ...
        $taxName = $salesOrder->items->firstWhere('tax_name')?->tax_name ?? 'VAT';
        $taxRate = $salesOrder->items->firstWhere('tax_rate')?->tax_rate;

        $issued = $salesOrder->issued_at?->format('d/m/Y') ?? '—';
    @endphp
    ...
    <table>
        <tr>
            ...
            <td style="width:40%;" class="right">
                ...
                <div style="margin-top:4px; font-weight:700;"># {{ $salesOrder->sales_order_number }}</div>

                <table class="meta" style="margin-top:18px; width:100%;">
                    <tr>
                        <td class="label small">Issued Date :</td>
                        <td class="value">{{ $issued }}</td>
                    </tr>
                    <tr>
                        <td class="label small">Reference :</td>
                        <td class="value">{{ $salesOrder->reference ?? '—' }}</td>
                    </tr>
                    <tr>
                        <td class="label small">Quote # :</td>
                        <td class="value">{{ $salesOrder->quote_number ?? '—' }}</td>
                    </tr>
                    <tr>
                        <td class="label small">Status :</td>
                        <td class="value">{{ strtoupper((string) ($salesOrder->status ?? '—')) }}</td>
                    </tr>
                </table>
            </td>
        </tr>
                ...
                <div class="box">
                    ...
                    @if ($salesOrder->contact)
                        <div class="small muted mt-6">
                            {{ $salesOrder->contact->name }}{{ $salesOrder->contact->email ? ' • ' . $salesOrder->contact->email : '' }}
                        </div>
                    @endif
        ...
        <tr>
            ...
            <td style="width:40%;">
                <table style="width:100%; border-collapse:collapse;">
                    ...
                    @if ($discount > 0)
                        <tr>
                            <td style="padding:10px 12px; text-align:left; color:#6b7280;">Discount</td>
                            <td style="padding:10px 12px; text-align:right; font-weight:700;">- {{ $money($discount) }}
                            </td>
                        </tr>
                    @endif
    ...
    <div class="mt-24">
        ...
        @if (!empty($salesOrder->terms))
            <div class="mt-10">
                ...
                <div class="small pre">{{ $salesOrder->terms }}</div>
            </div>
        @endif
    </div>
